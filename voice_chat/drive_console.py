...
protocol and [console_model.py](console_model.py) for the pacing and the English.
tkinter ships with Python, which matters because this is a diagnostic, and a
diagnostic that needs something installed first is one you will not have when you
need it.

**There is a second window onto all of this, and it is the one to reach for.**
[drive_web.py](drive_web.py) serves the same console as a browser page, and it
exists because this one cannot be made to fit on a screen: every panel below is at
a fixed size in a fixed place, so the transcript and the turns table fall off the
bottom of a 1080p display with no scrollbar anywhere to reach them, and a wider
window only adds empty space beside the camera. A page scrolls and reflows. This
one stays because it needs no browser, because it is what measured everything
described here, and because the two share `console_model` -- so a change to the
pacing or the commentary is a change to both.
...
from console_model import (
    ALARM_WHEN_FALSE, ALARM_WHEN_TRUE, BATTERY_COLOURS, BATTERY_NOTES,
    BATTERY_POLL_S, BATTERY_STALE_S, CAMERA_AUTO_S, Channel, CLEAR_ARM_S,
    LIGHT_MAX, LOG_LINES, LOUD_PHASES, MAP_AUTO_S, MAP_EXTENTS_M, MAP_LEGEND,
    MAP_SIZES_PX, MOVE_TIMEOUT_S, POLL_S, Reply, STATUS_FIELDS, TRACK_POLL_S,
    TURN_PRESETS_DEG, TURN_ROWS, WIFI_COLOURS, WIFI_POLL_S, WIFI_REJOIN_MS,
    WIFI_SCAN_TIMEOUT_S, move_sentence, or_dash, rung, tap_to_relative,
    wifi_verdict, worth_logging)

# Names this module has always used, kept as they were. The definitions moved to
# [console_model.py](console_model.py) when the browser console needed the same
# sentences and the same pacing; the call sites here did not have to know.
_f = or_dash
_wifi_verdict = wifi_verdict
...
class Console:
    ...
    def _map_tap(self, event: tk.Event) -> str:
        ...
        if "drive_to" not in self.tools:
            self._say("this rover has no drive_to tool, so the tap was not sent\n",
                      "quiet")
            return "break"
        # The geometry is the renderer's own, through console_model, because it is
        # the exact inverse of the sampling that drew the picture.
        where = tap_to_relative(event.x, event.y, self.map_view)
        if where is None:
    ...
    def _zoom(self, step: int) -> None:
        ...
        index = rung(MAP_EXTENTS_M, self.half_extent)
    ...
    def _resize(self, step: int) -> None:
        ...
        index = rung(MAP_SIZES_PX, self.map_size)
