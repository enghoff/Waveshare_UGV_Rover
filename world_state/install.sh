...
#
# The llama.cpp binary is the released aarch64 build rather than one compiled
# here. This host has no CUDA toolkit and no cmake, the model is 2B and quantized,
# and nothing else deployed on this rover uses the GPU -- so a CPU build is what
# the runtime actually is, and building it locally would only add a toolchain to
# install and a half-hour to every rebuild.
...
if [ -n "$FORCE" ] || [ ! -x "$VENDOR/llama/llama-server" ]; then
    echo "--- fetching llama.cpp $LLAMA_BUILD"
    tarball="$VENDOR/llama-$LLAMA_BUILD.tar.gz"
    curl -fL --retry 5 --retry-delay 5 -o "$tarball" \
        "https://github.com/ggml-org/llama.cpp/releases/download/$LLAMA_BUILD/llama-$LLAMA_BUILD-bin-ubuntu-arm64.tar.gz"
    rm -rf "$VENDOR/llama"
