...
|---|---|---|
| 1 | [`Main.dc.html`](Main.dc.html) | the ROS 2 graph that drives the rover: the hardware and the daemon outside ROS, the three nodes written here, and `slam_toolbox` with the seven Nav2 servers |
| 2 | [`WorldState.dc.html`](WorldState.dc.html) | the semantic world state: a look through the gimbal camera, the three models that measure it, and how two of those looks become a thing with a place on the map |
