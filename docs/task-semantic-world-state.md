...
| | |
|---|---|
| the camera was blind | Auto-exposure had wound up for a dark room and never came back down, so every frame was white and the first attempt at the drive recorded regions that were not there — a spray bottle, a rug, a fan, in a room holding two armchairs. Forcing manual at the shortest exposure fixed it, 0 regions to 30. **That reading of it was wrong and the workaround made things worse.** It recurred on 2026-09-03, and what was found was the camera still pinned to that hand-set exposure a day later — a v4l2 control is the driver's state and outlives the program that set it, the device being closed, and a reboot. Aperture Priority Mode was never the problem: nothing on the rover was putting the exposure back. A capture now reads the controls first and hands the camera back its own exposure if anyone is holding it, and a blank frame is retaken rather than returned — see `face_tracking/uvc_camera.py`. **The lesson is the one worth keeping: a blank frame reads exactly like an empty room.** "0 of 0 regions kept" and "nothing to place" are both things the rover says when it is working perfectly, so a whole drive was recorded off white frames before anyone looked at one. |
